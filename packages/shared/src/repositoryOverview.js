...
function detectFrameworks(packageJson) {
    ...
    const detected = FRAMEWORK_DETECTIONS.filter((entry) => entry.packages.some((packageName) => packageNames.has(packageName))).map((entry) => entry.label);
    return detected.slice(0, 8);
}
function inferRepositoryShape(topLevelEntries, frameworks) {
...
async function extractReadmeSummary(repoPath) {
...
function inferPurpose(packageJson, readmeSummary, repositoryShape, frameworks) {
...
async function detectCoverageSummary(repoPath, files) {
    ...
    for (const candidate of candidates) {
        const summary = await readJsonFile(path.join(repoPath, candidate));
        if (summary?.total) {
            return {
...
function buildCoverageSummary(coverageSummary, files, scripts) {
    ...
    if (files.some((file) => file.toLowerCase().includes("coverage") &&
...
function buildAnswer(options) {
    const languageSummary = options.topLanguages.length > 0
        ? options.topLanguages
            .slice(0, 3)
            .map((entry) => entry.label)
            .join(", ")
        : "no dominant languages detected";
    const frameworkSummary = options.frameworks.length > 0
        ? options.frameworks.join(", ")
        : "no major frameworks detected from the root package.json";
    const riskSummary = options.risks.length > 0
    ...
    return [
        ...
        `I found ${options.testFileCount} likely test file${options.testFileCount === 1 ? "" : "s"}. ${options.coverageSummaryText}`,
        riskSummary,
    ].join(" ");
}
export async function generateRepositoryOverview(repoPath) {
    const files = [];
    ...
    const packageJson = await readJsonFile(path.join(repoPath, "package.json"));
...
export function formatRepositoryOverviewSummary(overview) {
    const purpose = overview.sections.find((section) => section.title === "Purpose");
    const languages = overview.sections.find((section) => section.title === "Languages");
    const testing = overview.sections.find((section) => section.title === "Testing and coverage");
    const risks = overview.sections.find((section) => section.title === "Observed gaps and issues");
    const milestones = overview.sections.find((section) => section.title === "Suggested milestones");
    return [
        overview.answer,
